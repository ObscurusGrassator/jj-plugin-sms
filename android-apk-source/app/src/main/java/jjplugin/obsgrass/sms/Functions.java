...
public class Functions {
    ...
    public Result run(String methodName, JSONObject input) {
        ...
        else if (methodName.equals("getNewSMSs"))
            return getNewSMSs(input);
        else if (methodName.equals("getContactByName"))
            return getContactByName(input);
        else if (methodName.equals("getContactByNumber"))
            return getContactByNumber(input);
        else {
    ...
    public String numberNorm(String number) {
        String result = Pattern.compile("^0+|[^0-9]", Pattern.DOTALL | Pattern.CASE_INSENSITIVE).matcher(number)
                .replaceAll("");
        return result.equals("") ? number : result;
    }

    public String nameNorm(String name) {
        name = Pattern.compile("[\\(\\)\\.\\[\\]]|^ +| +$", Pattern.DOTALL | Pattern.CASE_INSENSITIVE).matcher(name)
                .replaceAll("");
    ...
    public Result getContactByNumber(JSONObject input) {
        try {
            ...
            if (contact == null)
                ...
                return new Result(
                        "{\"number\": \"" + contact.number + "\", \"fullName\": \"" + contact.fullName + "\"}");
        } catch (JSONException e) {
    ...
    public Contact getContactByNumber(Integer number) {
        return getContactByNumber(number.toString());
    }
    ...
    class Contact {
        ...
        public void addRatio(Integer add) {
            ratio = ratio + add;
        }
                    ...
                    while (pCur.moveToNext()) {
                        ...
                        for (String name2 : names) {
                            for (Integer i = name2.length(); i >= 3; i--) {
                                if (Pattern.compile(" " + name2.substring(0, i) + "|^" + name2.substring(0, i))
                                        .matcher(nameNorm(fullName)).find()) {
                                    contact.addRatio(i);
                                    break;
                                }
                            }
    ...
    public Result test(JSONObject input) {
        try {
            Log.d("~= test",
                    input.getString("name") + " " + input.getString("number") + " " + input.getString("message"));
            // sendSMS(new JSONObject("{\"number\":\"0915716413\",\"message\":\"Test
            // 2\"}"));
     ...
     * const filter = {
     * box: 'inbox', // 'inbox' (default), 'sent', 'draft', 'outbox', 'failed',
     * 'queued', and '' for all
     *
        ...
        try {
            ...
            for (int i = 0; i < sorted.length(); i++) {
                ...
                if (result.has(number))
                    ((JSONArray) ((JSONObject) result.get(number)).get("messages")).put((String) sms.get("body"));
                else {
                    ...
                    if (contact != null)
                        newi.put("fullName", contact.fullName);
